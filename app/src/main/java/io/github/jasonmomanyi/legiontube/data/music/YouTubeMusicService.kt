package io.github.jasonmomanyi.legiontube.data.music

import android.util.Log
import io.github.jasonmomanyi.legiontube.LegionTubeApplication
...
import io.github.jasonmomanyi.legiontube.player.stream.AudioStreamSelector
...
object YouTubeMusicService {
    ...
    suspend fun fetchTrendingMusic(limit: Int = 50): List<MusicTrack> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val innertubeTracks = io.github.jasonmomanyi.legiontube.data.newmusic.InnertubeMusicService.fetchTrendingMusic()
            if (innertubeTracks.isNotEmpty()) {
                ...
                val chartTracks = io.github.jasonmomanyi.legiontube.data.newmusic.InnertubeMusicService.fetchCharts()
            ...
            try {
                tracks.addAll(io.github.jasonmomanyi.legiontube.data.newmusic.InnertubeMusicService.fetchCharts())
    ...
    suspend fun fetchNewReleases(limit: Int = 30): List<MusicTrack> = withContext(Dispatchers.IO) {
        try {
            val innertubeResults = io.github.jasonmomanyi.legiontube.data.newmusic.InnertubeMusicService.searchMusic("new music releases 2025")
    ...
    suspend fun searchMusic(query: String, limit: Int = 50): List<MusicTrack> = withContext(Dispatchers.IO) {
        try {
            // Try Innertube first
            val innertubeResults = io.github.jasonmomanyi.legiontube.data.newmusic.InnertubeMusicService.searchMusic(query)
            if (innertubeResults.isNotEmpty()) {
                return@withContext innertubeResults.take(limit)
            }
     ...
     */
    suspend fun searchArtists(query: String, limit: Int = 10): List<io.github.jasonmomanyi.legiontube.ui.screens.music.ArtistDetails> = withContext(Dispatchers.IO) {
        try {
            val service = ServiceList.YouTube
    ...
    suspend fun getBestAudioStream(videoId: String): Pair<org.schabi.newpipe.extractor.stream.AudioStream, Long>? = withContext(Dispatchers.IO) {
        try {
            ...
            val preferredAudioLanguage = PlayerPreferences(LegionTubeApplication.appContext)
     ...
     */
    suspend fun fetchPlaylistDetails(playlistId: String): io.github.jasonmomanyi.legiontube.ui.screens.music.PlaylistDetails? = withContext(Dispatchers.IO) {
        ...
        try {
            if (playlistId.startsWith("MPREb_") || playlistId.startsWith("FEmusic_library_privately_owned_release_")) {
                val album = io.github.jasonmomanyi.legiontube.data.newmusic.InnertubeMusicService.fetchAlbum(playlistId)
                if (album != null) return@withContext album
            } else {
                val playlist = io.github.jasonmomanyi.legiontube.data.newmusic.InnertubeMusicService.fetchPlaylistDetails(playlistId)
        ...
        try {
            if (playlistId.startsWith("MPREb_") || playlistId.startsWith("FMDM") || playlistId.startsWith("OLAK")) {
                ...
                if (albumPage != null) {
                    ...
                    return@withContext io.github.jasonmomanyi.legiontube.ui.screens.music.PlaylistDetails(
                        id = playlistId,
            ...
            io.github.jasonmomanyi.legiontube.ui.screens.music.PlaylistDetails(
                id = playlistId,
    ...
    suspend fun fetchPlaylistContinuation(playlistId: String, continuation: String): Pair<List<MusicTrack>, String?> = withContext(Dispatchers.IO) {
        try {
            io.github.jasonmomanyi.legiontube.data.newmusic.InnertubeMusicService.fetchPlaylistContinuation(playlistId, continuation)
        } catch (e: Exception) {
    ...
    private fun convertSongItemToMusicTrack(item: io.github.jasonmomanyi.legiontube.innertube.models.SongItem): MusicTrack? {
        return MusicTrack(
            videoId = item.id,
            title = item.title,
    ...
    suspend fun getRelatedMusic(videoId: String, limit: Int = 20, audioOnly: Boolean = false): List<MusicTrack> = withContext(Dispatchers.IO) {
        try {
            val innertubeRelated = io.github.jasonmomanyi.legiontube.data.newmusic.InnertubeMusicService.getRelatedMusic(videoId, audioOnly)
     ...
     */
    suspend fun searchPlaylists(query: String, limit: Int = 10): List<io.github.jasonmomanyi.legiontube.ui.screens.music.MusicPlaylist> = withContext(Dispatchers.IO) {
        try {
            val innertubePlaylists = io.github.jasonmomanyi.legiontube.data.newmusic.InnertubeMusicService.searchPlaylists(query)
            ...
            searchExtractor.initialPage.items
                ...
                .take(limit)
                .map { item ->
                    io.github.jasonmomanyi.legiontube.ui.screens.music.MusicPlaylist(
    ...
    suspend fun fetchArtistDetails(channelId: String): io.github.jasonmomanyi.legiontube.ui.screens.music.ArtistDetails? = withContext(Dispatchers.IO) {
        try {
            ...
            val innertubeDetails = io.github.jasonmomanyi.legiontube.data.newmusic.InnertubeMusicService.fetchArtistDetails(channelId)
