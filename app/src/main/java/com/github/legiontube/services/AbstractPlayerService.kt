...
import com.github.legiontube.util.DefaultTrackSelectorWithAudioQualitySupport
import com.github.legiontube.util.NowPlayingNotification
import com.github.legiontube.util.PauseableTimer
...
abstract class AbstractPlayerService : MediaLibraryService(), MediaLibrarySession.Callback {
    ...
    open fun navigateVideo(videoId: String) {
        ...
        val wakeLock = powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "LegionTube::TransitionWakeLock")
        // Force the CPU to stay awake for 15 seconds max during network resolution. This fixes the background transition death!
    ...
    override fun onCreate() {
        ...
        // Manually start foreground immediately with a lightweight notification to prevent Android 12+ ForegroundServiceStartNotAllowedException
        val loadingNotification = NotificationCompat.Builder(this, LegionTubeApp.PLAYER_CHANNEL_NAME)
    ...
    private fun createPlayerAndMediaSession() {
        ...
        // prevent android from putting LegionTube to sleep when locked
