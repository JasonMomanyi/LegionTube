...
import io.github.jasonmomanyi.legiontube.ui.components.FeedInvalidationBus
...
import io.github.jasonmomanyi.legiontube.player.quality.QualityManager
...
import io.github.jasonmomanyi.legiontube.notification.UpcomingVideoReminderWorker
import io.github.jasonmomanyi.legiontube.utils.PerformanceDispatcher
import io.github.jasonmomanyi.legiontube.utils.parsePremiereTimestamp
...
import io.github.jasonmomanyi.legiontube.data.video.DownloadedVideo
import io.github.jasonmomanyi.legiontube.data.model.SponsorBlockSegment
import io.github.jasonmomanyi.legiontube.data.repository.SponsorBlockRepository
import com.google.gson.Gson
import com.google.gson.reflect.TypeToken
import io.github.jasonmomanyi.legiontube.ui.screens.player.util.VideoPlayerUtils
import io.github.jasonmomanyi.legiontube.ui.screens.player.util.VideoErrorMapper
...
class VideoPlayerViewModel @Inject constructor(
    ...
    private val playlistRepository: io.github.jasonmomanyi.legiontube.data.local.PlaylistRepository,
    ...
    private val _commentsState = MutableStateFlow<List<io.github.jasonmomanyi.legiontube.data.model.Comment>>(emptyList())
    val commentsState: StateFlow<List<io.github.jasonmomanyi.legiontube.data.model.Comment>> = _commentsState.asStateFlow()
    ...
    fun startBackgroundService() {
        ...
        if (videoId != null && audioUrl != null) {
            val musicTrack = io.github.jasonmomanyi.legiontube.ui.screens.music.MusicTrack(
                videoId = videoId,
    ...
    fun playNext() {
        ...
        if (!handledByPlayer) {
            viewModelScope.launch {
                val infiniteRadioEnabled = playerPreferences.infiniteRadioEnabled.first()
                val currentVideoId = _uiState.value.cachedVideo?.id ?: _uiState.value.streamInfo?.id
                val registry = io.github.jasonmomanyi.legiontube.data.recommendation.engines.EngineRegistry.getInstance(context)
                
                var nextVideo: Video? = null
                if (infiniteRadioEnabled && registry.isInfiniteRadioAvailable() && currentVideoId != null) {
                    val queue = registry.getInfiniteRadioQueue(currentVideoId, limit = 1)
                    nextVideo = queue.firstOrNull()
                }
                
                if (nextVideo == null) {
                    nextVideo = _uiState.value.relatedVideos.firstOrNull()
                }
                
                nextVideo?.let { video ->
                    playVideo(video)
                    io.github.jasonmomanyi.legiontube.player.GlobalPlayerState.setCurrentVideo(video)
                }
            }
        }
    }
    ...
    fun playPrevious() {
        ...
        if (!handledByPlayer) {
            getPreviousVideoId()?.let { prevId ->
                ...
                playVideo(prevVideo)
                io.github.jasonmomanyi.legiontube.player.GlobalPlayerState.setCurrentVideo(prevVideo)
            }
        }
    }
                ...
                kotlinx.coroutines.withTimeout(30_000) {
                    ...
                    if (streamInfo != null) {
                        ...
                        try {
                            ...
                            LegionTubeNeuroEngine.onVideoInteraction(context, video, InteractionType.CLICK)
                        } catch (e: Exception) {
                        ...
                        val pipedStream = io.github.jasonmomanyi.legiontube.network.PipedFallbackClient.getStreamInfo(videoId)
                        if (pipedStream != null) {
    ...
    private suspend fun preparePipedMediaForPlayback(
        videoId: String,
        pipedStream: io.github.jasonmomanyi.legiontube.network.PipedFallbackClient.PipedStreamResponse,
        savedPosition: Long,
        loadToken: Long
    ) = withContext(Dispatchers.Main) {
        if (!isPlaybackLoadCurrent(loadToken)) return@withContext
    ...
    fun reportWatchProgress(video: io.github.jasonmomanyi.legiontube.data.model.Video, position: Long, duration: Long) {
        ...
        viewModelScope.launch {
            LegionTubeNeuroEngine.onVideoInteraction(
                context,
                video,
                interactionType,
    ...
    fun likeVideo(videoId: String, title: String, thumbnail: String, channelName: String, channelId: String = "") {
        viewModelScope.launch {
            ...
            try {
                ...
                LegionTubeNeuroEngine.onVideoInteraction(context, video, InteractionType.LIKED)
                io.github.jasonmomanyi.legiontube.data.recommendation.engines.EngineRegistry
                    .getInstance(context)
                    .recordInteraction(video, InteractionType.LIKED)
            } catch (e: Exception) { }
        }
    }
    ...
    fun loadCommentReplies(comment: io.github.jasonmomanyi.legiontube.data.model.Comment) {
    ...
    fun loadMoreCommentReplies(comment: io.github.jasonmomanyi.legiontube.data.model.Comment) {
        val videoId = _uiState.value.streamInfo?.id ?: return
        val repliesPage = comment.repliesPage ?: return
