...
import io.github.jasonmomanyi.legiontube.data.model.ShortsSequenceResult
 ...
 *      b) Topic-driven searches using LegionTubeNeuroEngine's learned interests
...
class ShortsRepository private constructor(private val context: Context) {
    ...
    private suspend fun fetchDiscoveryFeed(): ShortsSequenceResult {
        ...
        val discoveryVideos: List<io.github.jasonmomanyi.legiontube.data.model.Video> =
    ...
    suspend fun loadMore(
        ...
        if (result != null && result.shorts.isNotEmpty()) {
            ...
            val recentlySeen = try {
                LegionTubeNeuroEngine.getRecentlySeenShorts()
            ...
            try {
                LegionTubeNeuroEngine.recordSeenShorts(reRanked.map { it.id })
            } catch (e: Exception) {
    ...
    private fun deduplicateByTitle(
        videos: List<io.github.jasonmomanyi.legiontube.data.model.Video>
    ): List<io.github.jasonmomanyi.legiontube.data.model.Video> {
        if (videos.size <= 1) return videos

        val result = mutableListOf<io.github.jasonmomanyi.legiontube.data.model.Video>()
    ...
    /**
     * Re-rank shorts using LegionTubeNeuroEngine.
     * 
    ...
    private suspend fun reRankWithFlowNeuro(
        ...
        return try {
            ...
            LegionTubeNeuroEngine.recordFeedImpressions(listOf(pinned.toVideo()) + ranked)
