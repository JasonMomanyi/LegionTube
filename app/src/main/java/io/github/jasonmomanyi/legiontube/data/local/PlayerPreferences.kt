...
import io.github.jasonmomanyi.legiontube.data.lyrics.PreferredLyricsProvider
import io.github.jasonmomanyi.legiontube.network.AppProxyConfig
import io.github.jasonmomanyi.legiontube.network.AppProxyType
...
class PlayerPreferences(context: Context) {
    ...
    private object Keys {
        ...
        // Spotify Algorithm Engine
        val SPOTIFY_ENGINE_ENABLED = booleanPreferencesKey("spotify_engine_enabled")
        val SPOTIFY_ENGINE_SEEN_PROMPT = booleanPreferencesKey("spotify_engine_seen_prompt")
        val INFINITE_RADIO_ENABLED = booleanPreferencesKey("infinite_radio_enabled")
    }
    ...
    // Spotify Algorithm Engine
    val spotifyEngineEnabled: Flow<Boolean> = context.playerPreferencesDataStore.data.map { it[Keys.SPOTIFY_ENGINE_ENABLED] ?: false }
    val spotifyEngineSeenPrompt: Flow<Boolean> = context.playerPreferencesDataStore.data.map { it[Keys.SPOTIFY_ENGINE_SEEN_PROMPT] ?: false }
    val infiniteRadioEnabled: Flow<Boolean> = context.playerPreferencesDataStore.data.map { it[Keys.INFINITE_RADIO_ENABLED] ?: false }
    ...
    // Spotify Algorithm Engine
    suspend fun setSpotifyEngineEnabled(enabled: Boolean) {
        context.playerPreferencesDataStore.edit { it[Keys.SPOTIFY_ENGINE_ENABLED] = enabled }
    }

    suspend fun setSpotifyEngineSeenPrompt(seen: Boolean) {
        context.playerPreferencesDataStore.edit { it[Keys.SPOTIFY_ENGINE_SEEN_PROMPT] = seen }
    }

    suspend fun setInfiniteRadioEnabled(enabled: Boolean) {
        context.playerPreferencesDataStore.edit { it[Keys.INFINITE_RADIO_ENABLED] = enabled }
    }
