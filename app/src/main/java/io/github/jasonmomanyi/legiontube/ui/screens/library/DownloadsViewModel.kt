package io.github.jasonmomanyi.legiontube.ui.screens.library

import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import io.github.jasonmomanyi.legiontube.data.music.DownloadManager as MusicDownloadManager
import io.github.jasonmomanyi.legiontube.data.local.entity.DownloadWithItems
import io.github.jasonmomanyi.legiontube.data.music.DownloadedTrack
...
import io.github.jasonmomanyi.legiontube.data.video.downloader.LegionTubeDownloadService
...
class DownloadsViewModel @Inject constructor(
            ...
            combine(
                ...
                val incomplete = downloads.filter { download ->
                    download.download.videoId !in pending &&
                        download.overallStatus != io.github.jasonmomanyi.legiontube.data.local.entity.DownloadItemStatus.COMPLETED
                }
    ...
    fun deleteVideoDownload(videoId: String) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (download?.overallStatus != io.github.jasonmomanyi.legiontube.data.local.entity.DownloadItemStatus.COMPLETED) {
                LegionTubeDownloadService.cancelDownload(appContext, videoId)
                delay(500L)
            }
    ...
    fun pauseVideoDownload(videoId: String) {
        LegionTubeDownloadService.pauseDownload(appContext, videoId)
    }

    fun resumeVideoDownload(videoId: String) {
        LegionTubeDownloadService.resumeDownload(appContext, videoId)
    }

    fun removeIncompleteDownloads() {
        viewModelScope.launch(Dispatchers.IO) {
            val ids = videoDownloadManager.allDownloads.first()
                .filter { it.overallStatus != io.github.jasonmomanyi.legiontube.data.local.entity.DownloadItemStatus.COMPLETED }
                .map { it.download.videoId }
            if (ids.isEmpty()) return@launch

            _pendingDeleteIds.update { it + ids }
            ids.forEach { videoId -> LegionTubeDownloadService.cancelDownload(appContext, videoId) }
            delay(500L)
