...
import io.github.jasonmomanyi.legiontube.data.local.QueuePersistence
import io.github.jasonmomanyi.legiontube.service.Media3MusicService
...
import io.github.jasonmomanyi.legiontube.data.local.AudioSettingsPersistence
...
object EnhancedMusicPlayerManager {
    ...
    suspend fun resolveStreamUrl(videoId: String): String? {
        ...
        return try {
             val playbackData = io.github.jasonmomanyi.legiontube.utils.MusicPlayerUtils.playerResponseForPlayback(videoId).getOrNull()
        ...
        controller.addListener(object : Player.Listener {
            ...
            override fun onPlayerError(error: androidx.media3.common.PlaybackException) {
                ...
                if (retryCount < MAX_RETRIES) {
                    ...
                    Log.e("EnhancedMusicPlayer", "Max retries reached. Playback failed.")
                    retryCount = 0
                }
            }
        })
    ...
    fun setEqProfile(profileName: String) {
        ...
        val baseProfile = io.github.jasonmomanyi.legiontube.data.model.EqPresets.presets[profileName] 
            ?: io.github.jasonmomanyi.legiontube.data.model.EqPresets.presets["Flat"]
            ?: io.github.jasonmomanyi.legiontube.data.model.ParametricEQ.createFlat()
            
        val boost = _bassBoostLevel.value
        val finalProfile = if (boost > 0) {
            val existingBandIndex = baseProfile.bands.indexOfFirst { 
                it.filterType == io.github.jasonmomanyi.legiontube.data.model.FilterType.LSC && 
            ...
            val newBands = if (existingBandIndex >= 0) {
                ...
                mutableBands.toList()
                 ...
                 val boostBand = io.github.jasonmomanyi.legiontube.data.model.ParametricEQBand(
                    60.0, 
                    boost.toDouble(), 
                    0.7, 
                    io.github.jasonmomanyi.legiontube.data.model.FilterType.LSC
                )
    ...
    fun applyEqProfile(profile: io.github.jasonmomanyi.legiontube.data.model.ParametricEQ) {
        try {
            val json = Json.encodeToString(io.github.jasonmomanyi.legiontube.data.model.ParametricEQ.serializer(), profile)
