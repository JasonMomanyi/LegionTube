package io.github.jasonmomanyi.legiontube.data.video.downloader
...
import io.github.jasonmomanyi.legiontube.MainActivity
import io.github.jasonmomanyi.legiontube.data.local.PlayerPreferences
import io.github.jasonmomanyi.legiontube.data.local.entity.DownloadFileType
...
import io.github.jasonmomanyi.legiontube.data.video.DownloadProgressUpdate
...
@AndroidEntryPoint
class LegionTubeDownloadService : Service() {
    ...
    companion object {
        private const val TAG = "LegionTubeDownloadService"
        const val CHANNEL_ID = "flow_downloads"
        const val NOTIFICATION_GROUP = "flow_download_group"
        
        const val ACTION_START_DOWNLOAD = "io.github.jasonmomanyi.legiontube.START_DOWNLOAD"
        const val ACTION_PAUSE_DOWNLOAD = "io.github.jasonmomanyi.legiontube.PAUSE_DOWNLOAD"
        const val ACTION_RESUME_DOWNLOAD = "io.github.jasonmomanyi.legiontube.RESUME_DOWNLOAD"
        const val ACTION_CANCEL_DOWNLOAD = "io.github.jasonmomanyi.legiontube.CANCEL_DOWNLOAD"
        ...
        fun startDownload(
            ...
            val intent = Intent(context, LegionTubeDownloadService::class.java).apply {
    ...
    private fun createNotification(
        ...
        if (!isComplete && !mission.isFailed()) {
            if (isMuxing) {
                ...
                if (mission.status == MissionStatus.PAUSED) {
                    // Show Resume button
                    val resumeIntent = Intent(this, LegionTubeDownloadService::class.java).apply {
                        action = ACTION_RESUME_DOWNLOAD
                        putExtra("video_id", videoId)
                    }
                    ...
                    // Show Pause button
                    val pauseIntent = Intent(this, LegionTubeDownloadService::class.java).apply {
                        action = ACTION_PAUSE_DOWNLOAD
                        putExtra("video_id", videoId)
                    }
                ...
                // Cancel button
                val cancelIntent = Intent(this, LegionTubeDownloadService::class.java).apply {
                    action = ACTION_CANCEL_DOWNLOAD
                    putExtra("video_id", videoId)
                }
